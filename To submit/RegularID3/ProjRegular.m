...
Num_Samples=300;
noise=0.05; %between 0 to 1: 0 no noise
...
Data=UCI_Data(1:250,:);
Data_Validate=UCI_Data(251:334,:);
%% Find the tree
%Data=TestModel();
%Find boundary with minimum Entropy
[X1min, idx1, Index1, X2min, idx2, Index2,pure, bX1, bX2]=FindMinEntropy(Data,Data(:,1));
...
[Clusters,LenClusters,Table]=TableWrite(Clusters,LenClusters,Table,Data,j,pure,Index1,Index2,idx1,idx2,X1min,X2min, bX1, bX2);
...
while (Allpure)
    ...
    temp=LenClusters(j);
    C=Clusters(j,1:temp);
    [X1min, idx1, Index1, X2min, idx2, Index2,pure, bX1, bX2]=FindMinEntropy(Data,C);
